...
public class TimelineResource {
    ...
    public ResponseEntity<TimelineDTO> createTimeline(@RequestBody TimelineDTO timelineDTO) throws URISyntaxException {
        ...
        if (timelineDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new timeline cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the timelineDTO couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
